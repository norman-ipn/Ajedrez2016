  ...
  while (i < 8)
      ...
      while (j < 8)
	{
	  switch (un_tablero->casillas[i][j])
	    ...
	    case 'p':
	      valor_de_jaque =
		validar_movimiento_peon (fila_K, columna_K);
	      if (valor_de_jaque == 1 )
		{
		  return 1;
		}
	    ...
	    case 'c':
	      valor_de_jaque =
		validar_movimiento_caballo(fila_K, columna_K);
	      if (valor_de_jaque == 1 )
		{
		  return 1;
		}
	    ...
	    case 't':
	      valor_de_jaque =
		valiidar_movimiento_torre (fila_K, columna_K);
	      if (valor_de_jaque == 1 )
		{
		  return 1;
		}
	    ...
	    case 'a':
	      valor_de_jaque =
		validar_movimiento_alfil  (fila_K, columna_K);
	      if (valor_de_jaque == 1 )
		{
		  return 1;
		}
	    ...
	    case 'q':
	      valor_de_jaque =
		validar_movimiento_reyna  (fila_K, columna_K);
	      if (valor_de_jaque == 1 )
		{
		  return 1;
		}
...
while(i<8){
  ...
  if(estado_blancas==1){
                      estado_blancas=estado_blancas+(validar_movimiento_rey(struct tablero *un_tablero,fila_K,columna_K,(fila_K+direccion[i][0]),(columna_K+direccion[i][1])));
                      if(estado_blancas==0){
...
if(estado_negras==1){
                      estado_negras=estado_negras+(validar_movimiento_rey(struct tablero *un_tablero,fila_k,columna_k,(fila_k+direccion[i][0]),(columna_k+direccion[i][1])));
                      if(estado_negras==0){
  ...
  int paso (struct tablero *un_tablero, int columna, int fila)	/* Esta función recibe las coordenadas de posición y regresa 1 si en ese posición hay una casilla blanca o negra (está vacío) y 0 si no */
  {
  ...
  int conocer_turno_jugador (int turno, struct tablero *un_tablero)	/*Esta función podria complementar la funcion girar tablero */
  {
    int mueve = 0;
    if ((turno % 2) == 0)
      {
  ...
  void invertir_tablero (struct tablero *un_tablero)	/*Esta función invierte el tablero como si se girara 180° grados */
  {
    char tempo[8][8];
    int i = 0;
    int j = 0;
  ...
  int obtener_posicion (int Columna, int Fila)	/* Esta función transforma la columna y la fila a un valor de 1 a 64, columna y fila tienen valor del 0 al 7(por ser de un arreglo de 8x8) */
  {
    int i = 0;
    int cuadrante = 0;
    Fila = Fila + 1;
  ...
  char color_remplazo (int columna, int fila)	/* Esta función regresa un caracter del color sobre el cual la pieza se encontraba antes de desplazarse */
  {
    int alterno = 0;
    int cuadrante = 0;
    int j = 1;
